...
async function run() {
  ...
  await withServers({}, async () => {
    ...
    assert.equal(landing.headers.get("x-relayer-release"), process.env.OPENCODE_ROUTER_RELEASE_ID || "v0.2.3")
  ...
  await withServers({ sessionCount: 1 }, async () => {
    ...
    assert.equal(health.data.release.releaseId, process.env.OPENCODE_ROUTER_RELEASE_ID || "v0.2.3")
